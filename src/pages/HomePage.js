...
const HomePage = () => {
  ...
  const handleSearch = (searchData) => {
    ...
    // กรองโรงแรมตามข้อมูลจาก HotelsPage
    const filteredHotels = HotelsPage.hotels.filter(hotel => {
      const matchesDestination = hotel.province.toLowerCase().includes(destination.toLowerCase()) ||
                                 hotel.name.toLowerCase().includes(destination.toLowerCase());
      return matchesDestination;
    });

    setSearchResults(filteredHotels);
    console.log('ผลการค้นหา:', filteredHotels);
  };

  const handleHotelClick = (hotelId) => {
    // เปลี่ยนไปหน้าโรงแรมที่มี id เป็น hotelId
            ...
            {searchResults.map((hotel, index) => (
              <Grid item xs={12} sm={4} key={index}>
                <DestinationCard 
                  name={hotel.name} 
                  image={hotel.image} 
                  onClick={() => handleHotelClick(hotel.id)} // เมื่อคลิกโรงแรมจะเปลี่ยนหน้าไปยังโรงแรม
                />
              </Grid>
            ))}
          </Grid>
